...
void setup() {
    ...
    pinMode(A1, OUTPUT);     // change this to 53 on a mega 
...
void loop() {
    ...
    if (rf12_recvDone()) {
      ...
      byte n = rf12_len;
        ...
        if (rf12_crc == 0) myFile = SD.open("uiot.txt" , FILE_WRITE);
        if (myFile) myFile.print((int) rf12_hdr);
        for (byte i = 0; i < n; ++i) {
            Serial.print(' ');
